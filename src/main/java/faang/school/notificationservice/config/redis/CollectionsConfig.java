...
public class CollectionsConfig {
    private final List<NotificationService> notificationServiceNexts;
    private final List<MessageBuilder> messageBuilders;

    @Bean
    public Map<UserDto.PreferredContact, NotificationService> notifications() {
        Map<UserDto.PreferredContact, NotificationService> notifications = new HashMap<>(notificationServiceNexts.size());
        for (NotificationService i : notificationServiceNexts) {
